...
// Replace existing generateMonthlyMemberCollectionReport in app.js
const startMarker = "// Print Monthly Member-wise Collection Report";
const altStartMarker = "function generateMonthlyMemberCollectionReport(";

if (code.includes(altStartMarker)) {
    const idx = code.indexOf(altStartMarker);
    const prevCommentIdx = code.lastIndexOf("// =", idx);
    const startIdx = prevCommentIdx !== -1 ? prevCommentIdx : idx;
    
    // Replace till the end of file or function
    code = code.substring(0, startIdx).trimEnd() + '\n\n' + newFunctionCode + '\n';
    fs.writeFileSync('app.js', code, 'utf8');
    console.log('✅ Replaced generateMonthlyMemberCollectionReport in app.js with Receipt-Based Collection Report');
} else {
    code += '\n\n' + newFunctionCode + '\n';
    fs.writeFileSync('app.js', code, 'utf8');
    console.log('✅ Appended new generateMonthlyMemberCollectionReport to app.js');
}
